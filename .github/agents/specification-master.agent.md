...
- Remit
  - This repo has no `SPEC.md` and no packages — the contract surface is the guides and tutorials in `docs/guides/` and `docs/tutorials/`, the project pages in `docs/projects/` and `docs/introduction/`, and the `docs/examples/**` corpus (and its `docs/examples/_expected/**` snapshots) those guides describe.
  - Prefer `docs/guides/living-doc-header-types.md`, `docs/guides/living-doc-document-types.md`, and `docs/guides/living-doc-glossary.md` as the format contracts; `docs/examples/README.md` documents the corpus's own sync obligation (a format change and a matching example update land in the same PR).
  - Must treat any doc-type rename or restructuring as a cascade — the same PR must update every guide, tutorial, and example that names the old shape, not just the guide where the change originates.
- Spec locations
