...
2. `packages/datasets_pdf` is renamed to a generator-agnostic name (e.g. `datasets_generator_ready` or `datasets_canonical`) in the same change — import paths and packaging metadata updated, with a temporary re-export shim if a consumer needs one. The package name never leaves `toolkit`, so this is churn rather than risk, and doing it alongside the artifact rename avoids leaving a `_pdf`-flavored name behind for the Markdown path to import from. *(Tracked as a [Roadmap](roadmap.md) Phase 1 task.)*
...
`collector-gh`'s planned `doc-source` and `ui-tests` modes should be implemented against that same format, including fields not yet reflected in `doc-issues-v1.0.0-schema.json` today (`not_in_scope`, AC-level precondition/not_in_scope extensions, deprecation and descoped-AC metadata, the `@AC:<id>/aspect:<value>` tag syntax, and Functionality-level header mining). This is a matter of extending the `generator-ready.json` (§5) and `doc-issues`/`doc-source` schemas, not a redesign. There are no live deployments during v1, so these schemas stay at `v1.0.0` and evolve in place as fields are added — they freeze at the v1 release, with no additive-`v1.x`-vs-`v2.0.0` boundary to reason about in the meantime. Tracked as a Phase 2 task — see [Roadmap](roadmap.md).
...
- [ ] In the same change, rename `toolkit/packages/datasets_pdf` to a generator-agnostic name (e.g. `datasets_generator_ready` or `datasets_canonical`) — update import paths and packaging metadata, keep a temporary re-export shim if any consumer needs one (§5 migration step 2).
