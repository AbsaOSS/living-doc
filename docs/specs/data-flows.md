...
## 9. The authoring-side source of truth: `agentic-toolkit`'s living-doc skill family

[`AbsaOSS/agentic-toolkit`](https://github.com/AbsaOSS/agentic-toolkit) is a general-purpose AI-skills library (Copilot/Claude/Cursor-compatible), not `living-doc`-specific — most of its skills (`pr-review`, `test-unit-write`, `python-standards`, …) are generic engineering practice, unrelated to this ecosystem. But it hosts one full skill family that *is* directly connected: the `@living-doc-bdd-copilot` agent, which dispatches across 12 skills (`living-doc-create-user-story`, `living-doc-create-feature`, `living-doc-create-functionality`, `living-doc-update`, `living-doc-impact-analysis`, `living-doc-gap-finder`, `living-doc-pageobject-scan`, `data-cy-instrument`, `living-doc-scenario-creator`, `bdd-maintain`, `gherkin-step`, `gherkin-living-doc-sync`) plus one non-task companion skill, `shared`, which holds the canonical entity/AC/ID grammar every other skill in the family imports from (`skills/shared/lib/ac_tag.py`, `living_doc_id.py`) and the reference docs that define the actual current format (`skills/shared/references/living-doc-glossary.md`, `living-doc-bdd-schemas.md`).

**This is the missing front of the pipeline.** Everything documented elsewhere in this spec and in [Architecture](architecture.md) starts at `collector-gh` mining GitHub Issues or `.feature` files that already exist. `agentic-toolkit`'s `living-doc-bdd-copilot` agent is what an engineer runs to *author* those User Story / Feature / Functionality entities and their Gherkin scenarios in the first place, in the exact directory/tag conventions `collector-gh/SPEC.md` was written to mine (`features/liv_doc_us/`, `features/liv_doc_func/`, `@US_ID:`, `@FUNC_ID:`, `@AC:` — down to the literal `liv_doc_us` directory-naming convention appearing identically in both repos, with no evident coordination between them).

### 9.1 `collector-gh/SPEC.md`'s format is now behind the authoring-side format

`skills/shared/references/living-doc-glossary.md` + `living-doc-bdd-schemas.md` is the newer, more complete, actively-maintained description of the entity/AC/header-block format — per your framing, treat it as authoritative over `collector-gh/SPEC.md`'s (older, already-drifted) version of the same thing. Concrete deltas, all confirmed by direct comparison of the two documents:

| In `agentic-toolkit`'s current format | In `collector-gh/SPEC.md` (as currently written) |
|---|---|
| AC states: `planned \| in_review \| active \| deprecated`, always lowercase | Examples show `Active` capitalized (`AC:US-27-01 (v1.9.0 - Active)`); `in_review` isn't mentioned at all |
| Feature-level `not_in_scope:` field, inherited by all ACs | No `not_in_scope` field anywhere in the header parsing rules (§3.6) |
| AC-level `preconditions:`/`not_in_scope:` extensions, nested under an individual AC block | AC block parsing (§3.6.7) only extracts `id`/`version`/`state`/`description` — no nested per-AC fields |
| `Aspect: <value1>, <value2>` and `<placeholder-name>: <value1>, <value2>` keyword lines inside an AC block, distinct from the description | §3.6.7 joins *every* `- ` line inside an AC into one prose description — would currently mis-parse a placeholder/value line as more description text |
| Deprecation metadata: `deprecated_at`, `deprecation_reason`, `superseded_by` | Not present in `doc-issues-v1.0.0-schema.json` or the SPEC's parsing rules at all |
| Descoped-AC handling (`descoped_at`, `descoped_reason`, `future_release`, state stays `PLANNED`) | No descoped-AC concept |
| `@AC:<id>/aspect:<value>` param syntax on scenario tags (canonical grammar: `AC_TAG_PATTERN` in `ac_tag.py`, `(?:US\|FUNC)-\d+-\d{2}(?!\d)` with optional `/param:value` segments) | §4.6.3 extracts `ac_ids[]` as bare values with no `/param` handling — a scenario tagged `@AC:US-1-01/aspect:username-input` would currently be captured as one malformed literal string, not parsed into ID + aspect |
| A full header-block format for **Functionality** entities (`# LIVING DOC — FUNC-<nnn> · ...`, with `parent: FEAT-<nnn>`, `func_type:` enum) | No mining path defined for Functionality-level header blocks at all — `ui_tests` mode only extracts `@FUNC_ID:` as a scenario-level file tag, never a Functionality entity's own header |
| Canonical AC-ID grammar is strictly `US-<n>-<nn>` or `FUNC-<n>-<nn>` — explicitly **not** `FEAT-...` ("Features own Functionalities, not ACs directly") | Consistent with this — no conflict here, just confirms `FEAT` entities never appear in an AC ID |

None of this is `collector-gh`'s fault — its `SPEC.md` predates (or was written independently of) `agentic-toolkit`'s current glossary, and the two repos show no sign of having coordinated. But it does mean: **implementing `collector-gh`'s `doc-source`/`ui-tests` modes exactly as `SPEC.md` currently describes them would ship a collector that already can't correctly parse what the authoring agent produces** — silently dropping `not_in_scope`, AC-level preconditions, deprecation metadata, and mis-parsing any `/aspect:`-tagged scenario. This should be fixed in the spec before Phase 2 implementation starts, not discovered after.

### 9.2 What this means for the canonical schema

The richer entity model in `agentic-toolkit`'s glossary is the real shape of the data this ecosystem needs to carry end to end — `generator-ready.json` (§5) and the `doc-issues`/`doc-source` schemas both need the same fields `collector-gh/SPEC.md` is currently missing (§9.1's left column) once the collector is updated to match. This is a schema *version* question, not a redesign — `doc-issues-v1.0.0` → `v2.0.0` (or additive `v1.x` if every new field can be optional) once `not_in_scope`, deprecation metadata, and AC-level precondition extensions are added.
