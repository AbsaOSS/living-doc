...
`collector-gh` and `generator-pdf`'s copies are close to each other but diverged (different phrasing throughout, e.g. collector-gh: *"Must keep sections in the order defined in this file"* vs generator-pdf: *"Must keep sections ordered exactly as listed in this file"* — same rule, reworded). `generator-mdoc` and `utilities`, meanwhile, share a **structurally different, more elaborate specification-master.agent.md** — with sections neither `collector-gh` nor `generator-pdf`'s copy has at all: "Verbosity levels" (brief/standard/detailed spec length tiers by change size), a "Minimum structure (portable)" checklist for what a spec document should contain, and a "Verification plan" deliverable alongside acceptance criteria. This is a materially better version of the same agent — and it exists in exactly two of the four repos that have this file, with no indication either lineage knows the other has diverged this far.
...
- [ ] Pick one canonical version of `copilot-instructions.md` — recommend starting from the `generator-mdoc`/`utilities` lineage for `specification-master.agent.md` specifically (it's the more complete of the two), and reconciling `copilot-instructions.md`/`copilot-review-rules.md` core-rule wording into one text, keyword dialect included (settle on `Must` / `Must not` / `Prefer` / `Avoid`, since it's the more common of the three dialects found and matches what the files' own "Structure" section already claims).
...
- [ ] Roll the reconciled template out to all 7 `living-doc-*` repos, including adding the full `.github/agents/` board to `toolkit` and `collector-ad` (currently missing it) and to `generator-markdown` once it has real code.
